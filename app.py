...
with col1:
    ...
    if st.button("📊 Открыть когортный анализ", use_container_width=True, type="primary", key="cohort_nav"):
        # Используем правильный формат для Streamlit Pages (относительно главного скрипта)
        try:
            # Формат: путь относительно главного скрипта, включая папку pages/
            st.switch_page("pages/1__Когортный_анализ.py")
        except Exception as e:
            # Если не работает, пробуем без расширения
            try:
                st.switch_page("pages/1__Когортный_анализ")
            except:
                st.error(f"Не удалось перейти на страницу. Используйте боковое меню слева для навигации.")
