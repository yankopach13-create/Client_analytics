...
tools = [
    ...
    (
        "Матрица вклада в период",
        "https://recency-contribution-matrix.streamlit.app/",
        "Инструмент для анализа вклада продаж в выбранный период по давности предыдущей покупки. Сегментация клиентов по месяцу последней покупки до первого дня анализируемого периода, оценка доли вклада по выбранной метрике. Анализ предыдущей покупки: для выбранных сегментов показывает топ категорий/товаров последнего дня покупки клиентов.",
    ),
    (
        "Анализ клиентов в продуктах",
        "https://cpa-statement.streamlit.app/",
        "Инструмент для анализа клиентской базы в разрезе продуктов.",
    ),
]
